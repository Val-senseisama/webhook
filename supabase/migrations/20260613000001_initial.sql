...
CREATE INDEX idx_events_status         ON events (status) WHERE status NOT IN ('delivered');
...
CREATE TABLE endpoints (
    ...
    name         TEXT NOT NULL,
    url          TEXT NOT NULL,
    secret       TEXT NOT NULL,           -- used to sign outgoing deliveries
...
CREATE INDEX idx_deliveries_pending     ON deliveries (next_attempt_at) WHERE status = 'pending';
